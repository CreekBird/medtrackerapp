...
class MedicationModelTests(TestCase):
    ...
    def test_adherence_rate_partial_adherence(self):
        ...
        now = timezone.now()
        DoseLog.objects.create(medication=med, taken_at=now - timedelta(hours=8), was_taken=True)
    ...
    def test_adherence_rate_no_logs(self):
        ...
        med = Medication.objects.create(name="Metformin", dosage_mg=500, prescribed_per_day=2)
        adherence = med.adherence_rate()
    ...
    def test_adherence_rate_all_doses_missed(self):
        ...
        med = Medication.objects.create(name="Lisinopril", dosage_mg=10, prescribed_per_day=1)

        now = timezone.now()
        DoseLog.objects.create(medication=med, taken_at=now - timedelta(days=1), was_taken=False)
        DoseLog.objects.create(medication=med, taken_at=now, was_taken=False)
    ...
    def test_expected_doses_zero_prescribed_per_day(self):
        ...
        med = Medication.objects.create(name="TestDrug", dosage_mg=50, prescribed_per_day=1)
    ...
    def test_adherence_rate_over_period_valid_dates(self):
        ...
        DoseLog.objects.create(medication=med, taken_at=base_date - timedelta(days=2), was_taken=True)
        DoseLog.objects.create(medication=med, taken_at=base_date - timedelta(days=1), was_taken=False)
    ...
    def test_adherence_rate_over_period_start_after_end(self):
        ...
        self.assertIn("start_date must be before or equal to end_date", str(context.exception))
...
class DoseLogModelTests(TestCase):
    ...
    def test_str_dose_missed(self):
        ...
        med = Medication.objects.create(name="Ibuprofen", dosage_mg=200, prescribed_per_day=1)
        now = timezone.now()
    ...
    def test_foreign_key_relationship(self):
        ...
        med = Medication.objects.create(name="Vitamin C", dosage_mg=1000, prescribed_per_day=1)
    ...
    def test_cascade_delete(self):
        ...
        med = Medication.objects.create(name="Atorvastatin", dosage_mg=20, prescribed_per_day=1)
        DoseLog.objects.create(medication=med, taken_at=timezone.now())
        DoseLog.objects.create(medication=med, taken_at=timezone.now() - timedelta(hours=1))
